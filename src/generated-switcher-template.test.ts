...
describe('renderNavControlComponent', () => {
    ...
    it('renders a submit button for each deck inside the dropdown menu', () => {
        ...
        expect(output).toContain('formaction="http://localhost:3000/__bridge?folder=advanced"');
        expect(output).toContain('formaction="http://localhost:3000/__bridge?folder=deep-dive"');
...
describe('renderContextMenuSetup', () => {
    it('generates a no-op context menu passthrough module', () => {
        const output = renderContextMenuSetup(makeOptions());

        expect(output).toContain('export default (items) => items;');
    });

    it('does not include switch-deck menu items anymore', () => {
        const output = renderContextMenuSetup(makeOptions());

        expect(output).not.toContain('Switch Deck');
        expect(output).not.toContain('form.submit()');
    });
    ...
    it('produces identical output for the same input', () => {
        const opts = makeOptions();
        expect(renderContextMenuSetup(opts)).toBe(renderContextMenuSetup(opts));
    });
});
